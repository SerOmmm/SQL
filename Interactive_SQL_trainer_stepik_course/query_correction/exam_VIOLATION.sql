...
нарушили одно и то же правило два и более раз. При этом учитывать все нарушения, независимо 
от того оплачены они или нет. Информацию отсортировать в алфавитном порядке, сначала по фамилии 
...
WHERE (f.name, f.number_plate, f.date_violation) = (p.name, p.number_plate, p.date_violation);

/* Создать новую таблицу back_payment через запрос, куда внести информацию о неоплаченных штрафах 
